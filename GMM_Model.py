...
def evaluate_synthetic_data(real_df, synthetic_df, discrete_features=None):
    features = real_df.columns
    num_features = len(features)
    ncols = 3
    nrows = (num_features + ncols - 1) // ncols
    
    plt.figure(figsize=(ncols * 8, nrows * 6))  # Adjusted figure size for clarity
    ...
    plt.tight_layout()
    plt.savefig('distributions.png', dpi=300, bbox_inches='tight', pad_inches=0.2)
    plt.close()
    ...
    if numeric_cols:
        ...
        if len(numeric_cols) < 7:
            ...
            plt.tight_layout()
            plt.savefig('correlation_matrix.png', dpi=300, bbox_inches='tight', pad_inches=0.2)
            plt.close()
...
def generate_synthetic_data(data, n_samples=1000, preserve_correlations=True, 
                          discrete_threshold=0.05, model_type='gmm'):
    ...
    ks_results = evaluate_synthetic_data(data, synthetic_df, discrete_features)
...
def main():
    ...
    synthetic_data, quality_metrics = generate_synthetic_data(
        data, n_samples=20000, model_type='gmm'
    )
    ...
    synthetic_data.to_csv(f'{name}_SYNTHETIC.csv', index=False)
# =============================================================================
